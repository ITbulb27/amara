...
export default function MensPage() {
  ...
  useEffect(() => {
    async function fetchProducts() {
      const { data, error } = await supabase.from('products').select('*').eq('category', 'mens')
  ...
  return (
    <div style={{ fontFamily: "'Segoe UI', system-ui, sans-serif", minHeight: '100vh', background: '#f9fafb' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #1a1a2e 0%, #be185d 100%)', padding: '3rem 2rem', textAlign: 'center', color: '#fff' }}>
        <p style={{ fontSize: '0.75rem', letterSpacing: '0.15em', textTransform: 'uppercase', color: '#fbcfe8', marginBottom: '0.5rem' }}>Amara's Collection</p>
